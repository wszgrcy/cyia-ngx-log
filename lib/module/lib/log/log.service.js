...
var cyia_ngx_common_1 = require("cyia-ngx-common");
...
var LogService = /** @class */ (function () {
     ...
     * @memberof LogService
     */
    LogService.prototype.setPrintLabel = function (label) {
        for (var x in label) {
    ...
    LogService.prototype.start = function (component, desc) {
        ...
        try {
            ...
            console.group(this.printOutLabelStr(this.label.start, this.timeHeap[this.timeHeap.push({ time: Date.now(), thisname: name_1, this: component, desc: desc }) - 1]));
        }
        catch (error) {
            console.group("" + desc);
        }
    ...
    LogService.prototype._printOut = function (method) {
        ...
        param.forEach(function (val) {
            type = cyia_ngx_common_1.TypeJudgment.getType(val);
            switch (type) {
                case cyia_ngx_common_1.jsNativeType.object:
                case cyia_ngx_common_1.jsNativeType.array:
                    ...
                    break;
                case cyia_ngx_common_1.jsNativeType.number:
                case cyia_ngx_common_1.jsNativeType.string:
                    _this.dataStr += val;
                    break;
                default:
                    _this.dataStr += '%o';
    ...
    LogService.prototype.containFunction = function (type, param) {
        if (type == cyia_ngx_common_1.jsNativeType.object) {
            return this._searchObject(param);
        }
        else if (type == cyia_ngx_common_1.jsNativeType.array) {
    ...
    LogService.prototype._searchObject = function (param) {
        for (var x in param) {
            if (param.hasOwnProperty(x)) {
                var element = param[x];
                if (cyia_ngx_common_1.TypeJudgment.getType(element) == cyia_ngx_common_1.jsNativeType.object) {
                    return this._searchObject(element);
                }
                else if (cyia_ngx_common_1.TypeJudgment.getType(element) == cyia_ngx_common_1.jsNativeType.array) {
                    return this._searchArray(element);
                }
                else if (cyia_ngx_common_1.TypeJudgment.getType(element) == cyia_ngx_common_1.jsNativeType.function) {
                    return true;
                }
            }
        }
    ...
    LogService.prototype.printOutLabelStr = function (label, obj) {
